...
final class FirstPersonViewmodelEditorWorkspace extends JDialog {
    ...
    FirstPersonViewmodelEditorWorkspace(
        ...
        this.modelBrowser = modelBrowser == null ? ignored -> {
        } : modelBrowser;
        this.appliedCallback = appliedCallback == null ? ignored -> {
        } : appliedCallback;
    ...
    private static void populateCombo(JComboBox<String> combo, List<String> values) {
        String selected = comboText(combo);
        combo.removeAllItems();
        values.forEach(combo::addItem);
        setCombo(combo, selected);
    }
    ...
    private static String selectedId(JList<CatalogEntry> list) {
        CatalogEntry entry = list.getSelectedValue();
        return entry == null ? "" : entry.id();
    }
    ...
    private static String uniqueId(String base, Set<String> existing) {
        ...
        int suffix = 2;
        while (existing.contains(normalized + "_" + suffix)) suffix++;
    ...
    private void deleteSet() {
        ...
        markDirty();
        refreshCatalogs();
        ensureSelection(Kind.ANIMATION_SET);
    }
    ...
    private void deleteProfile() {
        ...
        markDirty();
        refreshCatalogs();
        ensureSelection(Kind.ITEM_PROFILE);
    }
    ...
    private void replaceRig(String oldId, FirstPersonCombatLibrary.RigDefinition updated) {
        ...
        LinkedHashMap<String, FirstPersonCombatLibrary.RigDefinition> rigs = new LinkedHashMap<>(draft.rigs());
        rigs.remove(oldId);
        rigs.put(updated.rigId(), updated);
    ...
    private void replaceProfile(String oldId, FirstPersonCombatLibrary.ItemProfile updated) {
        ...
        profiles.put(updated.itemId(), updated);
    ...
    private void markDirty() {
        if (loading) return;
        dirty = true;
        stateLabel.setText("Unsaved draft");
    }
    ...
    private JPanel browseRow(JTextField field) {
        ...
        browse.addActionListener(event -> modelBrowser.accept(field));
    ...
    private JList<CatalogEntry> listFor(Kind kind) {
        return switch (kind) {
            case RIG -> rigList;
            case ANIMATION_SET -> setList;
            case ITEM_PROFILE -> profileList;
        };
    }

    private String itemLabel(String id) {
        ItemOption item = itemsById.get(id);
        return item == null ? "Unavailable " + id : item.displayName();
    }
    ...
    private final class RigForm {
        ...
        void load(FirstPersonCombatLibrary.RigDefinition rig) {
            if (rig == null) return;
            loading = true;
            id.setText(rig.rigId());
            name.setText(rig.displayName());
            model.setText(rig.modelPath());
            leftArm.setText(rig.defaultLeftArmPath());
            rightArm.setText(rig.defaultRightArmPath());
            setCombo(leftShoulder, rig.leftShoulderBone());
            setCombo(leftElbow, rig.leftElbowBone());
            setCombo(leftHand, rig.leftHandBone());
            setCombo(rightShoulder, rig.rightShoulderBone());
            setCombo(rightElbow, rig.rightElbowBone());
            setCombo(rightHand, rig.rightHandBone());
            ...
            px.setValue(rig.positionX());
            py.setValue(rig.positionY());
            pz.setValue(rig.positionZ());
            rx.setValue(rig.rotationX());
            ry.setValue(rig.rotationY());
            rz.setValue(rig.rotationZ());
            scale.setValue(rig.scale());
            fov.setValue(rig.fieldOfViewDegrees());
            near.setValue(rig.nearPlane());
            crossfade.setValue(rig.crossfadeMs());
            fallbacks = rig.fallbackBindings();
            loading = false;
        ...
        private void inspectArmModel(boolean left) {
            ...
            new SwingWorker<ArmInspection, Void>() {
                @Override
                protected ArmInspection doInBackground() throws Exception {
        ...
        private void inspectRigModel() {
            ...
            new SwingWorker<ModelInspection, Void>() {
                @Override
                protected ModelInspection doInBackground() throws Exception {
                    return inspect(path);
                }
                ...
                protected void done() {
                    try {
                        ...
                        populateCombo(leftShoulder, info.nodes());
                        populateCombo(leftElbow, info.nodes());
                        populateCombo(leftHand, info.nodes());
                        populateCombo(rightShoulder, info.nodes());
                        populateCombo(rightElbow, info.nodes());
                        populateCombo(rightHand, info.nodes());
                        ...
                        autoDetectBones(info.nodes());
                        autoDetectMeshes(info.meshes());
    ...
    private final class AnimationSetForm {
        ...
        AnimationSetForm() {
            row(panel, "Stable ID", id);
            row(panel, "Display Name", name);
            row(panel, "Compatible Rig", rig);
            slots.setVisibleRowCount(9);
            ...
            navigation.add(previousUsed);
            navigation.add(nextUsed);
            slotChooser.add(navigation, BorderLayout.SOUTH);
            row(panel, "Action Slot", slotChooser);
            row(panel, "Enabled", enabled);
            row(panel, "Animation Model", browseRow(path));
            ...
            cameraY.setToolTipText(cameraHelp);
            cameraZ.setToolTipText(cameraHelp);
            cameraRotationX.setToolTipText(cameraHelp);
            cameraRotationY.setToolTipText(cameraHelp);
            cameraRotationZ.setToolTipText(cameraHelp);
            ...
            makeDefault.addActionListener(event -> {
                ...
                EnumMap<WeaponType, String> defaults = new EnumMap<>(WeaponType.class);
                defaults.putAll(draft.weaponDefaults());
                defaults.put(type, loadedSetId);
                draft = new FirstPersonCombatLibrary.Content(draft.defaultRigId(), draft.rigs(),
        ...
        void load(FirstPersonCombatLibrary.AnimationSet set) {
            ...
            loading = true;
            id.setText(set.id());
            name.setText(set.displayName());
            refreshReferences();
            selectComboEntryPreserving(rig, set.rigId(), "Unavailable rig");
            bindings.clear();
            bindings.putAll(set.bindings());
            ...
            slots.setSelectedValue(initial, true);
            loadedSlot = slots.getSelectedValue();
            slots.repaint();
            loading = false;
            loadBinding(loadedSlot);
        }
        ...
        void commit() {
            ...
            if (!requestedId.equals(loadedSetId) && draft.animationSets().containsKey(requestedId)) {
                ...
                id.setText(loadedSetId);
                requestedId = loadedSetId;
            }
        ...
        private void loadBinding(FirstPersonCombatLibrary.AnimationSlot slot) {
            ...
            cameraX.setValue(camera.positionX());
            cameraY.setValue(camera.positionY());
        ...
        private void updateBindingEnabled() {
            ...
            path.setEnabled(active);
            clip.setEnabled(active);
            speed.setEnabled(active);
            impact.setEnabled(active);
            cameraX.setEnabled(active);
            cameraY.setEnabled(active);
            cameraZ.setEnabled(active);
            cameraRotationX.setEnabled(active);
            cameraRotationY.setEnabled(active);
        ...
        private void resetCameraFraming() {
            ...
            cameraY.setValue(0.0);
            cameraZ.setValue(0.0);
            cameraRotationX.setValue(0.0);
            cameraRotationY.setValue(0.0);
    ...
    private final class ProfileForm {
        ...
        ProfileForm() {
            ...
            row(panel, "Right Coverage", rightCoverage);
            ...
            usePreview.addActionListener(event -> {
                ...
                for (int index = 0; index < previewItem.getItemCount(); index++) {
                    if (previewItem.getItemAt(index).id().equals(selected.id())) {
                        previewItem.setSelectedIndex(index);
                        break;
                    }
                }
                commit();
        ...
        void load(FirstPersonCombatLibrary.ItemProfile profile) {
            ...
            pz.setValue(socket.positionZ());
            rx.setValue(socket.rotationX());
            ry.setValue(socket.rotationY());
            rz.setValue(socket.rotationZ());
            scale.setValue(socket.normalizedHeight());
            secondaryX.setValue(profile.secondaryGripX());
            secondaryY.setValue(profile.secondaryGripY());
            secondaryZ.setValue(profile.secondaryGripZ());
            leftArmor.setText(profile.leftArmorPath());
            rightArmor.setText(profile.rightArmorPath());
        ...
        void refreshReferences() {
            ...
            item.removeAllItems();
            items.forEach(value -> item.addItem(new CatalogEntry(value.id(), value.displayName())));
            rig.removeAllItems();
            draft.rigs().values().forEach(value ->
                    rig.addItem(new CatalogEntry(value.rigId(), value.displayName())));
            set.removeAllItems();
            draft.animationSets().values().forEach(value ->
